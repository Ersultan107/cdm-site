...
async function renderAll() {
    try {
        // Запрашиваем только ПЕРВЫЙ лист (gid=0), где ты всё объединил
        ...
        if (teamDiv) {
            teamDiv.innerHTML = rows.slice(1).map(r => (r.c && r.c[0] && r.c[0].v) ? `
        ...
        // 2. РЕЙТИНГ (Колонки E, F)
        const leaderboardDiv = document.getElementById('leaderboard');
        if (leaderboardDiv) {
            leaderboardDiv.innerHTML = rows.slice(1).map((r, i) => (r.c && r.c[4] && r.c[4].v) ? `
        ...
        // 3. КВЕСТЫ (Колонки H, I)
        const questDiv = document.getElementById('quests-list');
        if (questDiv) {
            questDiv.innerHTML = rows.slice(1).map(r => (r.c && r.c[7] && r.c[7].v) ? `
